...
def classement(epreuve):
    ...
    while z<=n:
        p = db.session.query(Participant).filter_by(id_epreuve = epreuve,hc=False,serie=z).filter(Participant.temps_init >= app.config['TMP_CHARGE_CHRONO'])\
            .order_by(desc(Participant.etat)).order_by(Participant.points_init).order_by(Participant.points_barr).order_by(Participant.points_barr2).all()
        tot = len(p)
        print tot
        delta = 1
...
def getDesc():
    return 'Barème A avec tour du vainqueur'